...
import logging
from saml2.sigver import REQ_ORDER, rm_xmltag
...
def http_form_post_message(message, location, relay_state="",
    ...
    if typ == "SAMLRequest" or typ == "SAMLResponse":
        _msg = base64.b64encode(message.encode("utf8")).decode("utf8")
    else:
        _msg = message
...
def make_soap_enveloped_saml_thingy(thingy, header_parts=None):
    ...
    if isinstance(thingy, str):
        ...
        body.append(_child)
        _bytes = ElementTree.tostring(envelope, encoding="UTF-8")
