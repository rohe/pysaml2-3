...
def _import_attrs(modul, tag, top):
    ...
    properties = [PyAttribute(key, val[0], top, True, obj.c_namespace, val[2],
                            val[1]) for key,val in list(obj.c_attributes.items())]
    for child in obj.c_child_order:
        for key, val in list(obj.c_children.items()):
            (pyn, mul) = val
            maximum = 1
...
def block_items(objekt, block, eldict):
    if objekt not in block:
        ...
        if isinstance(objekt, PyType):
            others = [p for p in list(eldict.values()) if isinstance(p, 
...
class Schema(Complex):
    ...
    def adjust(self, eldict, block):
        ...
        keys = [k.name for k in list(udict.keys())]
        print("#", keys)
        res = (None, [])
        if not udict:
            return res
        ...
        while True:
            non_child = 0
            for objekt, (sup, elems) in list(udict.items()):
                if sup:
                    continue
                else:
                    non_child += 1
            ...
            if res[0]:
                ref = res[0].name
                tups = res[1]
                for objekt, (sups, elems) in list(udict.items()):
                    if sups:
                        for sup in sups:
                            if sup.name == ref:
                                for tup in tups:
    ...
    def out(self):
        ...
        for _namespace, (mod, namn) in list(self.impo.items()):
            if namn:
...
for nsp in NAMESPACE_BASE:
    for nskey, func in list(_MAP.items()):
...
def read_schema(doc, add, defs, impo, modul, ignore, sdir):
    ...
    for key, namespace in list(tree._root.attrib["xmlns_map"].items()):
