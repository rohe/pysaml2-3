...
def test_ava_filter_1():
    ...
    assert _eq(list(ava.keys()), ["givenName", "mail"])
...
def test_assertion_2():
    AVA = {'mail': 'roland.hedberg@adm.umu.se',
...
def test_filter_values_req_opt_4():
    ...
    print(ava)
    assert _eq(list(ava.keys()), ['givenName', 'sn'])
...
def test_filter_ava_0():
    ...
    assert _eq(list(ava.keys()), ["givenName", "surName", "mail"])
    assert ava["givenName"] == ["Derek"]
    assert ava["surName"] == ["Jeter"]
...
def test_filter_ava_4():
    ...
    assert _eq(list(ava.keys()), ['mail', 'givenName', 'surName'])
...
def test_filter_on_wire_representation_1():
    ...
    assert _eq(list(ava.keys()), ["sn", "givenname"])
...
def test_filter_on_wire_representation_2():
    ...
    ava = assertion.filter_on_wire_representation(ava, acs, r, o)
    assert _eq(list(ava.keys()), ["sn", "givenname", "title"])
