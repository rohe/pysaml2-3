#Since I ask this of people before using their code samples, anyone can use
# this under BSD.
import os
import M2Crypto
...
#If you don't like the default M2Crypto ASCII "progress" bar it makes when
# generating keys, you can use:
...
#The larger the key size the longer it will take to generate the key and the
# larger the signature will be when signing
...
CipherText = WriteRSA.public_encrypt("This is a secret message that can only be decrypted with Bob's private key",
...
#If you use the wrong private key to try to decrypt the message it generates an
# exception, so this catches the exception
try:
    PlainText = ReadRSA.private_decrypt(CipherText,
                                        M2Crypto.RSA.pkcs1_oaep_padding)
except:
...
#Tell it to verify our string, if this string is not identical to the one that
# was signed, it will fail
