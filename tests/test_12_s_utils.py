...
SUCCESS_STATUS = """<ns0:Status xmlns:ns0="urn:oasis:names:tc:SAML:2.0:protocol"><ns0:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Success" /></ns0:Status>"""

ERROR_STATUS = """<ns0:Status xmlns:ns0="urn:oasis:names:tc:SAML:2.0:protocol"><ns0:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:Responder"><ns0:StatusCode Value="urn:oasis:names:tc:SAML:2.0:status:UnknownPrincipal" /></ns0:StatusCode><ns0:StatusMessage>Error resolving principal</ns0:StatusMessage></ns0:Status>"""
...
def test_attribute_base64():
    b64sl = base64.b64encode("Selma Lagerlöf".encode("utf8")).decode("utf8")
...
def test_parse_attribute_map():
    ...
    assert _eq(list(forward.keys()), list(backward.values()))
    assert _eq(list(forward.values()), list(backward.keys()))
    print((list(forward.keys())))
    assert _oeq(list(forward.keys()), [
    ...
    assert _eq(list(forward.keys()), [
        ...
        ('urn:oid:0.9.2342.19200300.100.1.3', NAME_FORMAT_URI),
        ('urn:oid:2.5.4.12', NAME_FORMAT_URI)])
    assert _eq(list(backward.keys()),
               ["surName", "givenName", "title", "uid", "mail",
...
def test_attribute():
    a = utils.factory(saml.Attribute,
                      ...
                      name_format="urn:oasis:names:tc:SAML:2"
    ...
    a = utils.factory(saml.Attribute,
                      ...
                      name_format="urn:oasis:names:tc:SAML:2"
                                  ".0:attrname-format:uri",
                      attribute_value=[
                          saml.AttributeValue(text="member@example.com")])
...
def test_authn_context():
    accr = utils.factory(saml.AuthnContext,
                         text="urn:oasis:names:tc:SAML:2"
                              ".0:ac:classes:unspecified")
