...
def test_create_class_from_xml_string_nameid():
    ...
    assert _eq(list(kl.keys()), ['sp_provided_id', 'sp_name_qualifier',
...
def test_attribute_element_to_extension_element():
    ...
    assert _eq(list(ee.attributes.keys()), ['FriendlyName', 'Name', 'NameFormat'])
...
""")
    ...
    assert _eq(list(aes[0].attributes.keys()), [])
    ...
    assert _eq(list(nid.attributes.keys()), ["Format"])
